    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //dpy added
                    // var xx = await GenerateToken(model);
                    var tokenString = GenerateTokenString(model.Email);
                    //var tokenString = GenerateToken(model.Email);

                    //write token to local storage??  what about server session??

                    HttpContext.Session.SetString("AuthorizationToken", tokenString);
                    //var tokenString = HttpContext.Session.GetString("AuthorizationToken");

                    //add to cookie, session not persistent
